...
# Input: Image
# Generate visual_featurs and avg_features
...
# Input: hidden_sent: 1*512
#        avg_features :visual information n*2048
#        semantic_features n*10*512
# Output:
#        ctx=[vis;sem] n*512
#        alpha_vis->avg_features
#        alpha_se ->semantic_features
...
# Input: hidden_sent: 1*512
#        ctx :all information n*512
#        hidden_sent: 1*512
# Output:
#        topic n*512
#        p_stop->avg_features
#        hidden_sent_update: 1*512
#        hiddde_state(h,c)
...
# Input: topic n*512
#        caption n*10 -> embedding space 10*512 -> n*10*512
# Then:  topic+caption -> n*11*512
# Output:
#        words n*100
class WordLSTM(nn.Module):
#       input:topic: (4,1,512) captions: 4,10 embedding(100*512) 100个词 每个词 512维度
#       torch.Size([4, 10, 512]) 每个x对应10个词,每个词都是512维度
#       torch.Size([4, 11, 512]) input+词向量空间结合,每个x,对应11个词,每个词512
#       torch.Size([4, 11, 512]) 每个x有11个hidden 输出,每个输出512(定义)
#       获取最后一个隐藏层输出 将单词 从512 映射回100
    def __init__(self,
                 embed_size,
                 hidden_size,
                 vocab_size,
