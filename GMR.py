...
class GMR:
    ...
    def _init_model_path(self):
        ...
        print("_init_model_path",self.args.model_path)
    ...
    def _init_writer(self):
        print("_init_writer ",os.path.join(self.model_dir, 'logs.txt'))
    ...
    def _load_mode_state_dict(self):
        self.start_epoch = 0
        print("_load_mode_state_dict",self.args.load_model_path)
        try:
    ...
    def _init_visual_extractor(self):
        ...
        print("_init_visual_extractor visual_model_name={}  pretrained={} path={}".
              format(self.args.visual_model_name,
                     self.args.pretrained,self.args.load_visual_model_path))
    ...
    def _init_mlc(self):
        ...
        print("_init_mlc classes={}  sementic_features_dim={} fc_in_features=s{} k={} load_mlc_model_path={}, trained={} ".
              format(self.args.classes,
                     self.args.sementic_features_dim, self.extractor.out_features, self.args.k,
                     self.args.load_mlc_model_path,self.args.mlc_trained))
    ...
    def _init_co_attention(self):
        ...
        print("_init_co_attention version={}  embed_size={} hidden_size={} visual_size={} k={} momentum={} load_mlc_model_path={}, trained={} ".
              format(self.args.attention_version,
                     self.args.embed_size, self.args.hidden_size, self.extractor.out_features,
                     self.args.k,self.args.momentum,self.args.load_co_model_path,self.args.co_trained ))
    ...
    def _init_optimizer(self):
        print("_init_optimizer  learning_rate={}".format(self.args.learning_rate))
        return torch.optim.Adam(params=self.params, lr=self.args.learning_rate)

    def _init_scheduler(self):
        print("_init_scheduler patience={}".format(self.args.patience))
        scheduler = ReduceLROnPlateau(self.optimizer, 'min', patience=self.args.patience, factor=0.1)
        return scheduler

    def _init_logger(self):
        print("_init_logger {}".format(os.path.join(self.model_dir, 'logs')))
        logger = Logger(os.path.join(self.model_dir, 'logs'))
        return logger

    def _log(self,train_tags_loss,train_stop_loss,train_word_loss,train_loss,
             val_tags_loss,val_stop_loss,val_word_loss,val_loss,lr,epoch):
        info = {
            'train tags loss': train_tags_loss.detach().cpu().numpy(),
            'train stop loss': train_stop_loss.detach().cpu().numpy(),
            'train word loss': train_word_loss.detach().cpu().numpy(),
    ...
    def train(self):
        for epoch_id in range(self.start_epoch, self.args.epochs):
            ...
            self.writer.write(
                "[{} - Epoch {}] train loss:{} - val_loss:{} - lr:{}\n"
                    .format(self._get_now(),
                            epoch_id,
                            train_loss,
                            val_loss,self.optimizer.param_groups[0]['lr']))
            ...
            self._log(train_tags_loss=train_tag_loss,
                      ...
                      epoch=epoch_id)
            print("train_tags_loss = {} ,train_stop_loss= {},train_word_loss={}, train_loss={}"
                  .format(train_tag_loss/10000,train_stop_loss/10,train_word_loss,train_loss))
            print("epoch_id {} finished".format(epoch_id))
...
class GMR_att(GMR):
    ...
    def _init_sentence_model(self):
        ...
        print("_init_sentence_model version={}  embed_size={} hidden_size={} sentence_num_layers={} dropout={} momentum={} load_sentence_model_path={}, trained={} ".
              format(self.args.sent_version,
                     self.args.embed_size, self.args.hidden_size, self.args.sentence_num_layers,
                     self.args.dropout,self.args.momentum,self.args.load_sentence_model_path,self.args.sentence_trained ))
    ...
    def _init_word_model(self):
        ...
        print("_init_word_model vocab_size={}  embed_size={} hidden_size={} num_layers={} n_max={}  load_sentence_model_path={}, trained={} ".
              format(len(self.vocab),
                     self.args.embed_size, self.args.hidden_size, self.args.word_num_layers,
                     self.args.n_max,self.args.load_word_model_path,self.args.word_trained ))
        return model
    ...
    def _epoch_train(self):
        ...
        for i, (images, _, label, captions, prob) in enumerate(self.train_data_loader):
            ...
            # print("tags",tags)
            # print("labels",self._to_var(label, requires_grad=False))
            ...
            # print("i {} lamda_tag {} tag_loss{} lambda_stop {} stop_loss {} lamda_word {} word_loss {} "
            #       .format(i, self.args.lambda_tag,batch_tag_loss,
            #               self.args.lambda_stop,batch_stop_loss,
            #               self.args.lambda_word, batch_word_loss))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_file_list', type=str, default='./data/train.csv',
                        help='the path for train')
    parser.add_argument('--val_file_list', type=str, default='./data/val.csv',
                        help='the path for val')
    # transforms argument
    ...
    parser.add_argument('--saved_model_name', type=str, default='v1',
    ...
    parser.add_argument('--visual_model_name', type=str, default='densenet201',
                        help='CNN model name')
    parser.add_argument('--k', action='store_true', default=True,
